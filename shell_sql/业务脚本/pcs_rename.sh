...
chk_in_file()
{
    ...
    echo "${_in_file_pattern}"|awk -F"|" '{for(i=1;i<=NF;i++){print $i;}}'|grep -v ^$|while read v_in_file_pattern
    do
...
#/opt/mcb/pcs/var/tmp/.incs_cdr_file_rename_${g_instance}.lck
...
echo $$ > $PIDLock
    ...
    #/opt/mcb/pcs/conf/incs_cdr_file_rename_${g_instance}.conf    g_isntance=HW_ASN or SMS
    g_tag_file=${MCB_HOME}/${MCB_APPID}/conf/.${module}_${g_instance}.tag
    #opt/mcb/pcs/conf/.incs_cdr_file_rename_SMS.tag
    if [ ! -f ${conf_file} ];then
    ...
    if [ -z "${list_file}" ]; then
        ...
        do
                ...
                #${g_tag_file}  = /opt/mcb/pcs/conf/.incs_cdr_file_rename_SMS.tag ，创建一个当前时间的文件
                echo "find ${src_dir} -type f -name ${file_pattern} ! -newer ${g_tag_file} | sort"
                #${src_dir}=/opt/mcb/pcs/data/incs/upload/gsm    
                #${file_pattern} NGN*
                find ${src_dir} -type f -name "${file_pattern}" ! -newer ${g_tag_file} | sort | while read path_file
                do
                        src_file=$(basename ${path_file})
                        lgSetProcFileName "${src_file}"

                        chk_result=`chk_in_file "$src_file" "${in_file_pattern}"`
                        # in_file_pattern=NGN.*    $src_file=文件名
                        if [ "$chk_result" = "$src_file" ]; then

                           cp  ${path_file} ${arch_dir}/
                           if [ $? -ne 0 ]; then
                            ...
                            #$file_number :list文件的数量   string_file_number以seq_len定义的数字长度表示list文件的数量

                        
                            mv ${path_file} ${dest_dir}/${file_prefix}_${prov_cd}_${proc_dt}_${string_file_number}
                            if [ $? -eq 0 ]; then
