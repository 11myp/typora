#!/usr/bin/ksh
############################################################
#@FileName incs_cdr_file_rename.sh
#@Author   liuyf
#@Date     2018-03-26
#@Desc     rename cdr file name
#@Conf     incs_cdr_file_rename_${inst}.conf
#@Usage    incs_cdr_file_rename.sh inst1
# ##########################################################
#incs_cdr_file_rename.sh SMS
#src_file
#SY.002404.20210815.0143.NET   Mo_240_20210816_00000000
#
...
while [ $ps_index -le 3 ]; do
    if [ -r $PIDLock ]; then
        ...
        kill -0 $pid >/dev/null 2>&1   #kill -0 用来检查进程是否存在，存在则返回0，不存在则返回1
        if [ $? -eq 0 ]; then
...
dest_dir=$(cfGetConfigItem common dest_dir)
...
if [ -z "${dest_dir}" ]; then
    lgWriteLog SERIOUS "main" -1 "in [common], [dest_dir] not set!"
    exit 1
fi
if [ ! -d "${dest_dir}" ]; then
    lgWriteLog SERIOUS "main" -1 "in [common], [dest_dir] is not correct!"
    exit 1
fi
    ...
    if [[ ${file_prefix} == Mo ]]; then
        find ${src_dir} -type f -name "${file_pattern}" ! -newer ${g_tag_file} | sort | while read path_file; do
            ...
            if [ "$chk_result" = "$src_file" ]; then
                ...
                mv ${path_file} ${dest_dir}/${file_prefix}_${prov_cd}_${proc_dt}_${string_file_number}   #重命名
                if [ $? -eq 0 ]; then
                    echo "${path_file} ${dest_dir}/${file_prefix}_${prov_cd}_${proc_dt}_${string_file_number}" >>${list_file_by_day}
                    file_number=$(expr ${file_number} + 1)
                    lgWriteLog SUCC "Rename File" 0 "file [${src_file} rename to ${dest_dir}/${file_prefix}_${prov_cd}_${proc_dt}_${string_file_number}] re ame successfully!"
                else
                    lgWriteLog SERIOUS "Rename File" -1 "file [${src_file} rename to ${dest_dir}/${file_prefix}_${prov_cd}_${proc_dt}_${string_file_number}] rename failed!"
                    exit 2
                fi
            else
        ...
        find ${src_dir} -type f -name "${file_pattern}" ! -newer ${g_tag_file} | sort | while read path_file; do
            ...
            if [ "$chk_result" = "$src_file" ]; then
                ...
                ori_name_pre4=$(echo ${path_file} | awk -F'gsm/' '{print $2}' | cut -c 1-4)
                string_file_number=$(printf "%0${seq_len}d" $file_number)
                mv ${path_file} ${dest_dir}/${file_prefix}_${prov_cd}_${ori_name_pre4}_${proc_dt}_${string_file_number}
                if [ $? -eq 0 ]; then
                    echo "${path_file} ${dest_dir}/${file_prefix}_${prov_cd}_${ori_name_pre4}_${proc_dt}_${string_file_number}" >>${list_file_by_day}
                    file_number=$(expr ${file_number} + 1)
                    lgWriteLog SUCC "Rename File" 0 "file [${src_file} rename to ${dest_dir}/${file_prefix}_${prov_cd}_${ori_name_pre4}_${proc_dt}_${string_file_number}] re ame successfully!"
                else
                    lgWriteLog SERIOUS "Rename File" -1 "file [${src_file} rename to ${dest_dir}/${file_prefix}_${prov_cd}_${ori_name_pre4}_${proc_dt}_${string_file_number}] rename failed!"
                    exit 2
                fi
            else
                lgWriteLog ALARM "skip File" -1 "file [${src_file} is not in-file!mv to ${arch_dir}/${src_file}.unknown"
