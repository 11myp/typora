...
while [ 1 -eq 1 ]; do
    ...
    find ${src_dir} -type f -name "${file_pattern}" ! -newer ${g_tag_file} | sort | while read path_file; do
        ...
        if [ "$chk_result" = "$src_file" ]; then
            ...
            #原始话单第11、12位
                ori_string=`echo ${path_file:10:2}`    
                mv ${path_file} ${dest_dir}/${file_prefix}_${prov_cd}_${ori_string}_${proc_dt}_${string_file_number}
                if [ $? -eq 0 ]; then
                    echo "${path_file} ${dest_dir}/${file_prefix}_${prov_cd}_${ori_string}_${proc_dt}_${string_file_number}" >>${list_file_by_day}
                    file_number=$(expr ${file_number} + 1)
                    lgWriteLog SUCC "Rename File" 0 "file [${src_file} rename to ${dest_dir}/${file_prefix}_${prov_cd}_${ori_string}_${proc_dt}_${string_file_number}] re ame successfully!"
                else
                    lgWriteLog SERIOUS "Rename File" -1 "file [${src_file} rename to ${dest_dir}/${file_prefix}_${prov_cd}_${ori_string}_${proc_dt}_${string_file_number}] rename failed!"
                    exit 2
                fi
            else
