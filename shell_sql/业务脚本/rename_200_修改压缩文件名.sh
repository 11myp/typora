...
echo $$ > $PIDLock
    ...
    if [ -z "${list_file}" ]; then
        ...
        do
                ...
                find ${src_dir} -type f -name "${file_pattern}*.Z"|while read fn
                do
                   typeTag=`file ${fn} |grep compressed|wc -l`
                   if [[ ${fn} == *\.Z ]] && [[ ${typeTag}${} -eq 1 ]];then
                      uncompress -f ${fn}
                      echo ${fn}|awk -F'.' '{print "mv",$1,$0}'|sh
                done

                echo "find ${src_dir} -type f -name ${file_pattern} ! -newer ${g_tag_file} | sort"
                num=`find ${src_dir} -type f -name ${file_pattern} ! -newer ${g_tag_file} | sort|wc -l`
                num1=`find ${src_dir} -type f -name ${file_pattern}|sort|wc -l`
            echo $num $num1
                find ${src_dir} -type f -name "${file_pattern}" ! -newer ${g_tag_file} | grep -v "Z$" | sort | while read path_file
                do
