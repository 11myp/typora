...
>/app/mcb/incs/DELETED/zengtf/log/compare.${prov}
...
mm=`date +%M`
ss=`date +%S`
echo "${hh}:${mm}:${ss} ${prov}省通信服务器语音解码成功数量是：${pcsVoNum}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省文件服务器备份目录下语音数量是：${dfsVoNum}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省通信服务器短信解码成功数量是：${pcsMoNum}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省文件服务器备份目录下短信数量是：${dfsMoNum}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
...
if [ `expr ${VoPre} \> 0.99` -eq 0 ];then
   echo "${hh}:${mm}:${ss} 语音文件完整系数${VoPre}，遗失文件数${leakVoNums},暂不处理" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
   exit -1
else
   echo "${hh}:${mm}:${ss} 语音文件完整系数${VoPre}，遗失文件数${leakVoNums},可以补处理文件" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
   fi

if [ `expr ${MoPre} \> 0.99` -eq 0 ];then
   echo "${hh}:${mm}:${ss} 语音文件完整系数${MoPre}，遗失文件数${leakMoNums},暂不处理" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
   exit -1
else
   echo "${hh}:${mm}:${ss} 语音文件完整系数${MoPre}，遗失文件数${leakMoNums},可以补处理文件" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
fi
...
sqlplus incsdba/Cmit_incs0@${db_service_nm} 2>&1 > /app/mcb/incs/DELETED/zengtf/log/moCleanTmp.${prov} <<EOF
set feedback off
set echo off
set linesize 500
set pagesize 500
...
sqlplus incsdba/Cmit_incs0@${db_service_nm} 2>&1 > /app/mcb/incs/DELETED/zengtf/log/rmmCleanTmp.${prov} <<EOF
set feedback off
set echo off
set linesize 500
set pagesize 500
...
cat /app/mcb/incs/DELETED/zengtf/log/voCleanTmp.${prov}|grep 'Vo_' > /app/mcb/incs/DELETED/zengtf/log/cleanFiles.${prov}
cat /app/mcb/incs/DELETED/zengtf/log/moCleanTmp.${prov}|grep 'Mo_' >> /app/mcb/incs/DELETED/zengtf/log/cleanFiles.${prov}
cat /app/mcb/incs/DELETED/zengtf/log/rmmCleanTmp.${prov}|grep 'RMM' >> /app/mcb/incs/DELETED/zengtf/log/cleanFiles.${prov}

/app/mcb/incs/DELETED/zengtf/shell/cleanFiles.sh ${prov}

#比较数据库成功的数量和dfs上的文件，缺少的放文件
sqlplus incsdba/Cmit_incs0@${db_service_nm} 2>&1 > /app/mcb/incs/DELETED/zengtf/log/dbVoFilesTmp.${prov} <<EOF
set feedback off
set echo off
set linesize 500
set pagesize 500
...
sqlplus incsdba/Cmit_incs0@${db_service_nm} 2>&1 > /app/mcb/incs/DELETED/zengtf/log/dbMoFilesTmp.${prov} <<EOF
set feedback off
set echo off
set linesize 500
set pagesize 500
...
sqlplus incsdba/Cmit_incs0@${db_service_nm} 2>&1 > /app/mcb/incs/DELETED/zengtf/log/dbRmmFilesTmp.${prov} <<EOF
set feedback off
set echo off
set linesize 500
set pagesize 500
...
grep Mo_ /app/mcb/incs/DELETED/zengtf/log/dbMoFilesTmp.${prov}|sort|uniq > /app/mcb/incs/DELETED/zengtf/log/dbMoFiles.${prov}
grep Vo_ /app/mcb/incs/DELETED/zengtf/log/dbVoFilesTmp.${prov}|sort|uniq > /app/mcb/incs/DELETED/zengtf/log/dbVoFiles.${prov}
grep RMM /app/mcb/incs/DELETED/zengtf/log/dbRmmFilesTmp.${prov}|sort|uniq > /app/mcb/incs/DELETED/zengtf/log/dbRmmFiles.${prov}

comm -13 /app/mcb/incs/DELETED/zengtf/log/dbVoFiles.${prov} /app/mcb/incs/DELETED/zengtf/log/dfsVoList.${prov} > /app/mcb/incs/DELETED/zengtf/log/LeakFiles.${prov}
comm -13 /app/mcb/incs/DELETED/zengtf/log/dbMoFiles.${prov} /app/mcb/incs/DELETED/zengtf/log/dfsMoList.${prov} >> /app/mcb/incs/DELETED/zengtf/log/LeakFiles.${prov}
comm -13 /app/mcb/incs/DELETED/zengtf/log/dbRmmFiles.${prov} /app/mcb/incs/DELETED/zengtf/log/dfsRmmList.${prov} >> /app/mcb/incs/DELETED/zengtf/log/LeakFiles.${prov}

cat /app/mcb/incs/DELETED/zengtf/log/LeakFiles.${prov}|while read fileNm
do
  if [[ $fileNm == Vo* ]];then
...
curDbVoNums=`cat /app/mcb/incs/DELETED/zengtf/log/dbVoFilesTmp.${prov}|grep 'Vo_'|sort|uniq|wc -l`
curDbMoNums=`cat /app/mcb/incs/DELETED/zengtf/log/dbMoFilesTmp.${prov}|grep 'Mo_'|sort|uniq|wc -l`
curDbRmmNums=`cat /app/mcb/incs/DELETED/zengtf/log/dbRmmFilesTmp.${prov}|grep 'RMM'|sort|uniq|wc -l`

hh=`date +%H`
mm=`date +%M`
ss=`date +%S`
echo "${hh}:${mm}:${ss} ${prov}省${month}月份语音文件数${dfsVoNum},当前数据库成功处理文件数${curDbVoNums}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省${month}月份短信文件数${dfsMoNum},当前数据库成功处理文件数${curDbMoNums}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省${month}月份彩信文件数${dfsRmmNum},当前数据库成功处理文件数${curDbRmmNums}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
...
sqlplus incsdba/Cmit_incs0@${db_service_nm} 1>&2 > /app/mcb/incs/DELETED/zengtf/log/sttlResult.${prov}  <<EOF
        set linesize 200
...
sqlplus incsdba/Cmit_incs0@incsm 1>&2 > /app/mcb/incs/DELETED/zengtf/log/sttlResultM.${prov}  <<EOF
        set linesize 200
...
voiceProcessFileDt=`grep "^voiceProcessFileDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
voiceSttlFileDt=`grep "^voiceSttlFileDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
voiceSttlSttlDt=`grep "^voiceSttlSttlDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
voiceDailySttlDt=`grep "^voiceDailySttlDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
voiceMonthlyZgSttlDt=`grep "^voiceMonthlyZgSttlDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
voiceMonthlyQdSttlDt=`grep "^voiceMonthlyQdSttlDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
smsProcessFileDt=`grep "^smsProcessFileDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
smsCdrFileDt=`grep "^smsCdrFileDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
smsCdrSttlDt=`grep "^smsCdrSttlDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
smsDailySttlDt=`grep "^smsDailySttlDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
smsMonthlyZgSttlDt=`grep "^smsMonthlyZgSttlDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
smsMonthlyQdSttlDt=`grep "^smsMonthlyQdSttlDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
rmmProcessFileDt=`grep "^rmmProcessFileDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
rmmCdrFileDt=`grep "^rmmCdrFileDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
rmmCdrSttlDt=`grep "^rmmCdrSttlDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
rmmDailySttlDt=`grep "^rmmDailySttlDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
rmmMonthlyZgSttlDt=`grep "^rmmMonthlyZgSttlDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`
rmmMonthlyQdSttlDt=`grep "^rmmMonthlyQdSttlDt###" /app/mcb/incs/DELETED/zengtf/log/sttlResul*${prov}|awk -F'###' '{print $2}'`

echo "${hh}:${mm}:${ss} ${prov}省${month}月份结果如下：" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}

echo "${hh}:${mm}:${ss} ${prov}省${month}月份语音解码成功文件数${pcsVoNum},当前数据库成功处理文件数${dfsVoNum},缺失文件数${leakVoNums}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
...
echo "${hh}:${mm}:${ss} ${prov}省${month}月份短信解码成功文件数${pcsMoNum},当前数据库成功处理文件数${dfsMoNum},缺失文件数${leakMoNums}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省${month}月份短信处理记录表话单量(以文件名做日期):${smsProcessFileDt}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省${month}月份短信详单表话单量(以文件名做日期):${smsCdrFileDt}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省${month}月份短信详单表话单量(以账期日做日期)：${smsCdrSttlDt}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省${month}月份短信日结算表话单量：${smsDailySttlDt}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省${month}月份短信月结算表话单量(ZG)：${smsMonthlyZgSttlDt}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省${month}月份短信月结算表话单量(QD)：${smsMonthlyQdSttlDt}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}

echo "${hh}:${mm}:${ss} ${prov}省${month}月份数据库成功处理彩信文件数${dfsRmmNum}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省${month}月份彩信处理记录表话单量(以文件名做日期):${rmmProcessFileDt}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省${month}月份彩信详单表话单量(以文件名做日期):${rmmCdrFileDt}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省${month}月份彩信详单表话单量(以账期日做日期)：${rmmCdrSttlDt}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省${month}月份彩信日结算表话单量：${rmmDailySttlDt}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省${month}月份彩信月结算表话单量(ZG)：${rmmMonthlyZgSttlDt}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
echo "${hh}:${mm}:${ss} ${prov}省${month}月份彩信月结算表话单量(QD)：${rmmMonthlyQdSttlDt}" >> /app/mcb/incs/DELETED/zengtf/log/compare.${prov}
