...
configfile=$MCB_HOME/$MCB_APPID/conf/qb_exe.conf
logfile=$MCB_HOME/$MCB_APPID/var/log/qb_exe.log
...
function show_help
{
   if [[ "$proc" = '*' ]]
   then
      echo Usage: qb_exe.sh command [argument...]
      ...
      echo
      echo Type [qb_exe.sh command -help] for each command detail describe
      echo       
   else
...
function sumdb_err
{
        grep "unable to open file " $MCB_HOME/$MCB_APPID/var/tmp/.qb_exe_$proc_index.log
        if [ $? -ne 1 ]; then
...
#       grep "ERROR " $MCB_HOME/$MCB_APPID/var/tmp/.qb_exe_$proc_index.log |grep "ORA-" $MCB_HOME/$MCB_APPID/var/tmp/.qb_exe_$proc_index.log
#       if [ $? -ne 1 ]; then
...
#               return 1
#       fi
        grep "sql_ret_err" $MCB_HOME/$MCB_APPID/var/tmp/.qb_exe_$proc_index.log
        if [ $? -ne 1 ]; then
...
lgInit ${logfile} "COM" "qb_exe.sh" $1 "Y"
if [ "$?" -ne "0" ]; then
    echo "Init log file failed!"
    exit
fi    
lgWriteLog INFO "main" 0 "qb_exe.sh start!"
...
sqlplus -S /nolog 2>&1 >$MCB_HOME/$MCB_APPID/var/tmp/.qb_exe_$proc_index.log <<EOF
connect $conn_str
...
#ZengXH 20070929{
if (test $? -ne 0) then
   message="Can not access db or Can not write to $MCB_HOME/$MCB_APPID/var/tmp/.qb_exe_$proc_index.log"
   lgWriteLog SERIOUS "main" -1 "${proc_sql1[$proc_index]} ${message}!"
   echo "Failed! Can not access db or Can not write to $MCB_HOME/$MCB_APPID/var/tmp/.qb_exe_$proc_index.log"
   exit -1
fi
#ZengXH 20070929}
...
echo
lgWriteLog INFO "main" 0 "qb_exe.sh finished!"